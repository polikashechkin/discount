...
def response(request):
    try:
        ...
        cur = conn.cursor()
        cur.execute(f"update coupon set accept_date=sysdate, state=1 where state=0 and id=:0",[coupon_id])
        accepted = cur.rowcount != 0
        if accepted:
            s = f'insert into coupon_log (coupon_id, creation_time, info) values(:0, sysdate, :1)'
            cur.execute(s, [coupon_id, f'купон погашен'])
        conn.commit()
        conn.close()
        if not accepted:
            return f'Купон "{coupon_id}" недействитеный или уже погашен', '400 Unknown coupon'
        else:
            return ''
