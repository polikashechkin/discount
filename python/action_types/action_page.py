...
class СуммоваяСкидка(FormControl.Param):
    def __init__(self):
        super().__init__(Action.СУММОВАЯ_СКИДКА, 'Суммовая скидка', type='number')

    def print(self, page, cell):
        value = self.get_value(page)
        if value:
            cell.text(f'{value} баллов')
        else:
            cell.text(f'0 баллов')
...
БазовыеПараметры.append(СуммоваяСкидка())
