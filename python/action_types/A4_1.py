...
CLASS = 0
#PERCENT = True
DESCRIPTION= 'Процентная скидка на сопутствующие товары по весу'
ABOUT = '''
    ...
    Количество основных товаров определяется по весу основных товаров (целое значение суммы 
    всех весов). Соответственно, предполагается, что все осноные товары это товары весовые и в чеке 
    задается вес.
    '''
IS_AVAILABLE = True
...
class Calculator(ActionCalculator): 
    ...
    def calc(self, engine, check):
        ...
        for line in check.lines:
            ...
            #        continue
            вес_основных_товаров += line.qty
            сумма_товаров += line.qty * line.price
            уникальные_товары.add(line.product)

        количество_основных_товаров = int(вес_основных_товаров)
